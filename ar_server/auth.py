...
def get_firebase():
    if 'firebase' not in g:
        try:
            ...
            conf = current_app.config["FIREBASE_CONF"]
            cred = credentials.Certificate(conf)
            g.firebase = firebase_admin.initialize_app(cred)
            current_app.logger.debug("Init firebase Admin with file: %s", conf)
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if request.method == 'POST' and not (session and session.get('user_uid', False)):
                ...
                if not token:
                    content = jsonify({'reason': 'a valid token is missing'})
                ...
                try:
                    ...
                    content = jsonify({'reason': 'The token is not valid'})
                    ...
                    content = jsonify({'reason': 'The token has expired'})
                    return make_response(content,  401,  {'WWW.Authentication': 'Basic realm: "id token expired"'})

                except Exception as e:
                    content = jsonify({'reason': 'Error in authentication. Contact the system administrator'})
                ...
                #     content = jsonify({'reason': 'User not allowed to access this page'})
...
#             except Exception as e:
#                 content = jsonify({'reason': 'The token is not valid or has expired'})
#                 return make_response(content,  401,  {'WWW.Authentication': 'Basic realm: "id token expired"'})
...
#                     content = jsonify({'reason': 'You are too fast, wait {} seconds between requests'.format(min_time)})
