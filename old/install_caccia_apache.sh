...
rm -rf $INSTALL_DIR/*
cp -r ~/ar_server $INSTALL_DIR

echo "= Creating convenience VARS"
echo "alias cp-ar='sudo cp -r ~/ar_server $INSTALL_DIR'" >> ~/.bashrc
...
venv/bin/pip install -r ar_server/requirements.txt
...
sudo a2enmod ssl
cd $INSTALL_DIR/ar_server
mv server_conf/ar_server.wsgi $INSTALL_DIR
sudo mv server_conf/001-ar_server.conf /etc/apache2/sites-available/000-default.conf
sudo mv server_conf/001-ar_server-ssl.conf /etc/apache2/sites-available/000-default-ssl.conf
